...
let rec pp_command : type t. _ -> t command * t -> unit =
 fun ppf -> function
...
let rec prepare_command :
    type res. res command -> string * (Ezjsonm.value -> res option) =
  let open Option_syntax in
  function
  | Monitors -> ("j/monitors", parse_monitors)
  | Workspaces -> ("j/workspaces", parse_workspaces)
...
let send_command_raw :
 ...
 fun ~sw:_ ~env:_ ~command dispatch_socket ->
  ...
  try
    let* result = Socket.request_json dispatch_socket dispatch parse in
